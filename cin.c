...
/*   Updated: 2022/10/31 17:02:28 by mardolin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sortc(t_stack *stack_a, t_stack *stack_b, t_stack *temp)
{
	...
	temp->cn = init_cin(temp->cn, stack_a, temp);
	while (stack_a->size > (495))
	{
		find_n_set_a(stack_a, temp->cn, stack_b->j);
		min_max_val(stack_b);
		mov_stack_b_cin(stack_a, stack_b, temp);
		chunk_index_increment(stack_b);
	}
	temp->i = 0;
	if (stack_a->size == 5)
